...
## Overview
Create a `registry.json` so `@zod-to-form/react` can be installed via `npx shadcn add @zod-to-form/zod-form`. Two approaches: (1) a runtime `<ZodForm>` component registry item using real shadcn primitives, and (2) a bootstrapper registry item that drops a pre-configured `z2f.config.ts` for the CLI codegen pipeline. Both require migrating form wrapper names from deprecated `FormField/FormLabel/FormControl/FormMessage` to the current `Field/FieldLabel/FieldDescription/FieldError` pattern.
...
- Create a `registry:lib` item that installs `z2f.config.ts` pre-configured for shadcn
- The config uses `Field/FieldLabel/FieldDescription/FieldError` primitives and `SHADCN_FIELD_TYPES`
...
The `shadcnComponentMap` keys change from `FormField/FormLabel/FormDescription/FormMessage` to `Field/FieldLabel/FieldDescription/FieldError`. Mitigated by keeping old keys as deprecated aliases in `defaultComponentMap`.
...
**Tasks**:
1. [ ] Rename wrapper component keys in `defaultComponentMap` and `shadcnComponentMap` from `FormField/FormLabel/FormDescription/FormMessage` to `Field/FieldLabel/FieldDescription/FieldError`. Keep old keys as deprecated aliases. Update `FieldRenderer.tsx` to reference new keys.
2. [ ] Update `init.ts` autodiscovery to detect `Field/FieldLabel/FieldDescription/FieldError` exports alongside legacy `FormField/FormLabel` patterns.
3. [ ] Update codegen `formPrimitives` defaults in `packages/cli/src/templates.ts` to use `Field/FieldLabel` naming.
4. [ ] Create `registry.json` at project root with two items: `zod-form` (`registry:component`) and `zod-form-cli` (`registry:lib`), following shadcn registry schema.
...
7. [ ] Update all tests to use new wrapper component key names and verify deprecated aliases still resolve.

**Acceptance Criteria**:
- [ ] `shadcnComponentMap` uses `Field/FieldLabel/FieldDescription/FieldError` keys
- [ ] Old `FormField/FormLabel` keys still resolve via deprecated aliases — no runtime breakage
