...
6. [ ] Add unit tests for maxLength disable behavior and arrayConfig label resolution in `packages/react/tests/` and `packages/codegen/tests/`
7. [X] Update playground to reflect new button styling when shadcn preset is active (inherited from component map)

**Acceptance Criteria**:
- [ ] Add button is disabled when array items reach `z.array().max(N)` limit (runtime + codegen)
- [ ] Remove button is disabled when array items are at `z.array().min(N)` (already works, verify)
- [ ] shadcn preset renders styled array buttons (outline variant, small size)
- [ ] Custom labels via `arrayConfig: { addLabel: 'Add Item', removeLabel: 'Remove' }` in FormMeta work
- [ ] Codegen output includes disabled logic and respects labels
- [ ] No breaking changes to existing array field usage
