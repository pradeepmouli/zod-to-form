...
> = {
   ...
   * @useWhen Always set `components` — it is the only required field in `ZodFormsConfig`, and its `source` sub-field (the import path for your component module) is the only required sub-field within it; all other sub-options (`preset`, `overrides`, `fieldTemplate`) are optional and layer on top of that required baseline.
   *
   ...
   * @avoidWhen - Avoid setting `preset: 'shadcn'` when your component library does not follow Radix/shadcn conventions — it injects `controlled: true` and field-expression props for `Select`, `Checkbox`, and `Switch`; any controlled-mode components in your library that share those names will inherit those overrides unintentionally.
   ...
   * @useWhen Set `defaults` when you have multiple schemas and want a shared project-wide fallback for generation settings without repeating the same value in every `schemas.[key]` entry. The precedence chain is **not uniform** across properties — it varies by property:
   *
  ...
  include?: string[];
  /**
   * @pitfalls - Patterns match export **names**, not file paths — a pattern like `src/schemas/*.ts` will never match anything; put it in `include`/`exclude` only if it matches the exported identifier string.
