...
export const largeSchema = z
  .object({
    ...
    // Shipping address (6 fields, with cross-field validation)
    shippingAddress: z
      .object({
        street: z.string(),
        street2: z.string().optional(),
    ...
    // Emergency contacts (array of objects, 4 fields each)
    emergencyContacts: z
      .array(
        z.object({
          name: z.string().min(1),
    ...
    // Skills (array with refine)
    skills: z.array(z.string().min(1)).min(1).max(20),

    // Preferences (6 fields)
...
// ─── Valid data fixtures for validation benchmarks ──────────────────
