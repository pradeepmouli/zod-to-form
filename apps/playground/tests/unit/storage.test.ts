...
import { savePlaygroundState, loadPlaygroundState } from '../../src/lib/storage.ts';
...
describe('storage', () => {
  ...
  it('returns null when missing required fields', () => {
    localStorage.setItem('z2f-playground-state', JSON.stringify({ foo: 'bar' }));
    const loaded = loadPlaygroundState();
    expect(loaded).toBeNull();
  });
  ...
  it('preserves activeTab code value', () => {
    savePlaygroundState({
      editorContent: 'test',
      componentMap: 'default',
      activeTab: 'code',
    ...
    vi.advanceTimersByTime(600);
    const loaded = loadPlaygroundState();
    expect(loaded!.activeTab).toBe('code');
  });
});
