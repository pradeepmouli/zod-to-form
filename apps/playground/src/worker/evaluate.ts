...
export function evaluate(jsCode: string): EvalResult {
  ...
  try {
    ...
    if (result && typeof result === 'object' && 'schema' in result && isZodSchema(result.schema)) {
      const formRegistry =
        result.formRegistry &&
        typeof result.formRegistry === 'object' &&
        'get' in result.formRegistry
    ...
    return {
      ...
      error: {
        type: 'runtime',
        message:
          err instanceof Error ? err.message : String(err ?? 'Runtime error during evaluation')
      }
    };
  }
}
